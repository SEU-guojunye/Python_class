...
def Test(Basis, dBasis, EXAGauss, NonLinearF, Alpha, N, GaussX, GaussW, nVals, flag):
    timeBegin = time.time()
    Solution = list()
    L2Solution = list()
    ...
    timeEnd = time.time()
    print(flag, "积分耗时", timeEnd - timeBegin)
...
ExactU = lambda x: x**10 - 1
TestControl(ExactU, NonLinearF, 1)
# 基包含10次项后精度突然升高
